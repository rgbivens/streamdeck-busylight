...
let settings;
const defaultConfiguration = { color: 'green', blink: false, sound: 0, volume: 75 };
...
$SD.on('connected', (jsn) => {
    ...
    configuration = Object.assign({}, defaultConfiguration, settings);
    if (isDomReady) updateConfigurationUI();
});

function updateConfigurationUI() {
    document.getElementById('colorSelect').value = configuration.color || defaultConfiguration.color;
    document.getElementById('blinkToggle').checked = !!configuration.blink;
    document.getElementById('soundSelect').value = String(configuration.sound ?? defaultConfiguration.sound);
    document.getElementById('volumeSelect').value = String(configuration.volume ?? defaultConfiguration.volume);
}

function saveConfiguration() {
    settings = Object.assign({}, configuration);
...
$SD.on('sendToPropertyInspector', jsn => {
     ...
     */
     if (pl.hasOwnProperty('error')) {
        ...
        configuration = Object.assign({}, defaultConfiguration, pl);
        settings = Object.assign({}, configuration);
        if (isDomReady) updateConfigurationUI();
    }
});
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('colorSelect').addEventListener('change', (event) => {
        configuration.color = event.target.value;
        saveConfiguration();
    });
    document.getElementById('blinkToggle').addEventListener('change', (event) => {
        configuration.blink = event.target.checked;
        saveConfiguration();
    });
    document.getElementById('soundSelect').addEventListener('change', (event) => {
        configuration.sound = parseInt(event.target.value, 10);
        saveConfiguration();
    });
    document.getElementById('volumeSelect').addEventListener('change', (event) => {
        configuration.volume = parseInt(event.target.value, 10);
        saveConfiguration();
    });
});
 ...
 */
 ...
 function sendValueToPlugin(value) {
    console.log("sendValueToPlugin", value);
